...
pub fn matmul(mat: T, vec: T, dims: (usize, usize)) -> Result<T,String> {
    match &mat.ty {
        Ty::Array(n, row_type) if *n == dims.0 => {
            match (&**row_type, &vec.ty) {
                (Ty::Array(m1, elem_type1), Ty::Array(m2, elem_type2)) if *m1 == dims.1 && *m2 == dims.1 => {
                    match (&**elem_type1, &**elem_type2) {
                        (Ty::Field, Ty::Field) => {
                            Ok(T::new(Ty::Array(dims.0, Box::new(Ty::Field)), term![Op::PfLinear(dims.0, dims.1); mat.term.clone(), vec.term.clone()]))
                        },
                        _ => Err(()),
                    }
                },
        ...
        _ => Err(()),
    }.map_err(|_| "invalid types for matmul".to_string())
}
