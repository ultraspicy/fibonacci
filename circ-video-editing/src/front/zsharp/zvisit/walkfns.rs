...
pub fn walk_statement<'ast, Z: ZVisitorMut<'ast>>(
    ...
    match stmt {
        ...
        Print(a) => visitor.visit_print_statement(a),
...
pub fn walk_print_statement<'ast, Z: ZVisitorMut<'ast>>(
    visitor: &mut Z,
    prin: &mut ast::PrintStatement<'ast>,
) -> ZVisitorResult {
    for expression in prin.expressions.iter_mut() {
        visitor.visit_expression(expression)?;

    }
    visitor.visit_any_string(&mut prin.message)?;
    visitor.visit_span(&mut prin.span)
}
