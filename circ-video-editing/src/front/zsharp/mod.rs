...
use crate::ir::term::fmt::IntoPretty;
...
impl<'ast> ZGen<'ast> {
    fn new(
        ...
        let this = Self {
            ...
            print_function_timings: std::env::var("ZSHARP_FLAMEGRAPH")
                .map(|l| !l.is_empty())
                .unwrap_or(false),
        };
        this.circ
        ...
        match f_name {
            ...
            "matmul" => {
                if args.len() != 2 {
                    ...
                    Err(format!(
                        "Got {} generic args to EMBED/{}, expected 2",
                        generics.len(),
                        f_name
                    ))
                } else {
                    let vec = args.pop().unwrap();
                    let mat = args.pop().unwrap();
                    let m = const_int(generics.pop().unwrap())?.to_usize().unwrap();
                    let n = const_int(generics.pop().unwrap())?.to_usize().unwrap();
                    matmul(mat, vec, (n,m))
                }
            }
            "int_to_bits" => {
    ...
    fn function_call_impl_<const IS_CNST: bool>(
        ...
        if self.print_function_timings {
            let tag = format!(
                "{}::{}",
                f_path.as_path().file_stem().unwrap().to_str().unwrap(),
                f_name
            );
            println!("enter {}", tag);
        }
        ...
        let dur = time::Instant::now() - before;
        if dur.as_millis() > 50 {
            info!(
                "{} ms to process {} {:?}",
                dur.as_millis(),
    ...
    fn stmt_impl_<const IS_CNST: bool>(&self, s: &ast::Statement<'ast>) -> Result<(), String> {
        ...
        match s {
            ...
            ast::Statement::Print(e) => {
                let exprs = e.expressions.iter().map(|exp|  {
                    let val = self.expr_impl_::<false>(exp)?;
                    Ok(const_value_simple(&val.term)
                        .map(|v| format!("{}", v.clone().into_pretty()))
                        .unwrap_or_else(|| format!("{}", val)))
                }).collect::<Result<Vec<_>, String>>()?;

                println!("{}", SimpleCurlyFormat.format(&e.message.value, &exprs)
                    .map_err(|_| format!("Failed to format print message at {}", span_to_string(&e.span)))?);
