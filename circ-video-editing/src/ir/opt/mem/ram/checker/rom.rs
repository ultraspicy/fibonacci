...
pub fn lookup(
    ...
    let haystack_array = term(Op::Tuple, haystack.clone());
    let needles_array = term(Op::Tuple, needles.clone());
    let counts_pre = unmake_tuple(term![Op::ExtOp(ExtOp::Haboeck); haystack_array, needles_array]);
