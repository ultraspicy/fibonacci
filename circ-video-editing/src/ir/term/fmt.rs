...
impl DisplayIr for Op {
    fn ir_fmt(&self, f: &mut IrFormatter) -> FmtResult {
        match self {
            ...
            Op::PfDiv => write!(f, "/"),
            Op::PfLinear(n,m) => write!(f, "(pf_linear {n} {m})"),
...
/// For convert into an associated type with a prettier Display implementation
pub trait IntoPretty {
    /// Wrapper type
    type PrettyVersion : Display;
    /// Conversion method
    fn into_pretty(self) -> Self::PrettyVersion;
}
/// Pretty version of Value
pub struct PrettyValue(Value);
impl IntoPretty for Value {
    type PrettyVersion = PrettyValue;
    fn into_pretty(self) -> Self::PrettyVersion {
        return PrettyValue(self)
    }
}
impl Display for PrettyValue {
    fn fmt(&self, f: &mut Formatter) -> FmtResult {
        match &self.0 {
           Value::BitVector(bv) => write!(f, "{}", bv.uint()),
           Value::F32(fl) => write!(f, "{}", fl),
           Value::F64(fl) => write!(f, "{}", fl),
           Value::Int(i) => write!(f, "{}", i),
           Value::Bool(b) => write!(f, "{}", b),
           Value::Array(a) => write!(f, "[{}]", a.values().iter().map(|v| v.clone().into_pretty()).format(", ")),
           Value::Field(fi) => write!(f, "{}", fi),
           Value::Tuple(tup) => write!(f, "({})", tup.clone().into_vec().into_iter().map(|v| v.into_pretty()).format(", ")),
           Value::Map(_) => unimplemented!() // this can't show up in a print statement so who
                                             // cares
        }
