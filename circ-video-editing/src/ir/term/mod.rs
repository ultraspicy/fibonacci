...
pub enum Op {
    ...
    Witness(String),
    /// Linear function on proof elements
    PfLinear(usize, usize),
...
impl Sort {
    ...
    pub fn is_group(&self) -> bool {
        match self {
            ...
            Sort::F32 | Sort::F64 | Sort::Array(..) | Sort::Map(..) => false,
...
pub fn eval_op(op: &Op, args: &[&Value], var_vals: &FxHashMap<String, Value>) -> Value {
    match op {
        ...
        Op::PfLinear(n,m) => {
            let matrix = args[0].as_array().values().into_iter().map(|x| x.as_array().values().into_iter().map(|x| x.as_pf().clone()).collect::<Vec<_>>()).collect::<Vec<_>>();
            let vec = args[1].as_array();
            let ks = vec.key_sort.clone();
            let values = vec.values();
            let vs = values[0].sort();
            let array = values.into_iter().map(|x| x.as_pf().clone()).collect::<Vec<_>>();
            let mut output = Vec::with_capacity(*n);
            for i in 0..*n {
                let mut val = matrix[i][0].clone() * array[0].clone();
                for j in 1..*m {
                    val += matrix[i][j].clone() * array[j].clone();
                }
                output.push(Value::Field(val));
            }
            Value::Array(Array::from_vec(ks, vs, output))
        ...
        Op::IntSize => Value::BitVector(BitVector::new(Integer::from(args[0].as_int().significant_bits()), 32)),
...
///
/// * an tuple term
pub fn unmake_tuple(a: Term) -> Vec<Term> {
    let n = check(&a).as_tuple().len();
    (0..n)
        .map(|i| term(Op::Field(i), vec![a.clone()]))
        .collect()
}
...
where
    Integer: From<T>,
{
    leaf_term(Op::Const(Value::Int(int.into())))
}
