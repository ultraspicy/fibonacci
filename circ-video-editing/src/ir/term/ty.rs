...
fn check_dependencies(t: &Term) -> Vec<Term> {
    match &t.op() {
        ...
        Op::PfLinear(_,_) => vec![t.cs()[0].clone(), t.cs()[1].clone()],
        Op::IntSize => Vec::new(),
...
fn check_raw_step(t: &Term, tys: &TypeTable) -> Result<Sort, TypeErrorReason> {
    ...
    match &t.op() {
        ...
        Op::PfLinear(n,m) => {
            // could do more robust type checking here
            match get_ty(&t.cs()[1]) {
               Sort::Array(k, v, m2) if m == m2 => {
                   Ok(Sort::Array(k.clone(), v.clone(), *n))
               }
               Sort::Tuple(types) => {
                   let elem_type = &types[0];
                   let mut output_types = Vec::with_capacity(*n);
                   for _ in 0..*n  {
                        output_types.push(elem_type.clone())
                   }
                   Ok(Sort::Tuple(Into::into(output_types)))
               }
               _ => Err(TypeErrorReason::Custom("Expected Array or tuple".to_string()))
            }
        }
