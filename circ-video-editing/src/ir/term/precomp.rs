...
impl PreComp {
    ...
    pub fn topo_order(&self) -> FxHashMap<String, usize> {
        ...
        let mut stack: Vec<(Term, bool)> = self
            .outputs
            .iter()
            .map(|(name, t)| (leaf_term(Op::Var(name.clone(), check(t))), false))
            .collect();
        let mut visited: TermSet = Default::default();
        while let Some((t, cs_pushed)) = stack.pop() {
