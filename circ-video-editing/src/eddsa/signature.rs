//! Implementations for EdDSA signatures.
use core::fmt::Debug;
...
/// Structure for EDDSASignature
pub type EDDSASignature = InternalSignature;

/// Error type for signature operations; to do: add more error variants as needed.
#[derive(Debug, Clone)]
pub enum SignatureOpError {
    /// Invalid format
    InvalidFormat,
}

// Below is from https://github.com/dalek-cryptography/curve25519-dalek/blob/main/ed25519-dalek/src/signature.rs
// -*- mode: rust; -*-
//
...
#[inline(always)]
fn check_scalar(bytes: [u8; 32]) -> Result<Scalar, SignatureOpError> {
    ...
    if bytes[31] & 224 != 0 {
        return Err(SignatureOpError::InvalidFormat);
...
fn check_scalar(bytes: [u8; 32]) -> Result<Scalar, SignatureOpError> {
    match Scalar::from_canonical_bytes(bytes).into() {
        None => Err(SignatureOpError::InvalidFormat), // Err(InternalError::ScalarFormat.into()),
        Some(x) => Ok(x),
    }
}
...
impl InternalSignature {
    ...
    #[inline]
    #[allow(non_snake_case)]
    pub fn from_bytes(bytes: &[u8; SIGNATURE_LENGTH]) -> Result<InternalSignature, SignatureOpError> {
...
impl TryFrom<&Ed25519Signature> for InternalSignature {
    type Error = SignatureOpError; //SignatureError;

    fn try_from(sig: &Ed25519Signature) -> Result<InternalSignature, Self::Error> {
        InternalSignature::from_bytes(&sig.to_bytes())
    }
}

impl From<InternalSignature> for Ed25519Signature {
    fn from(sig: InternalSignature) -> Ed25519Signature {
        Ed25519Signature::from_components(*sig.R.as_bytes(), *sig.s.as_bytes())
    }
}
