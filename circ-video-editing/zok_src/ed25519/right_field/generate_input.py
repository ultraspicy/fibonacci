...
print("(set_default_modulus 57896044618658097711785492504343953926634992332820282019728792003956564819949")
print("(let (")
encoded_r, r_x, pk_bits, a, s, h, m_arr = mkinput(sig,m)
for i, bit in enumerate(pk_bits):
    print(f"(pk.{i} {'true' if bit else 'false'})")

for i,lbl in enumerate(["x","y","z","t"]):
    print(f"(a.{lbl} #f{a[i]})")

# commend out the below for vin file
#for i, bit in enumerate(encoded_r):
#  print(f"(encoded_r.{i} {'true' if bit else 'false'})")
#
#for i, bit in enumerate(r_x):
#  print(f"(r_x.{i} {'true' if bit else 'false'})")
#
#for i, bit in enumerate(s):
#  print(f"(s.{i} {'true' if bit else 'false'})")
#
