...
impl<E: Engine> SpartanCircuit<E> for FreivaldsConvCircuit<E::Scalar> {
  fn public_values(&self) -> Result<Vec<<E as Engine>::Scalar>, SynthesisError> {
    ...
    let flattened_edited_image : Vec<<E as Engine>::Scalar> = self.edited_image
      .clone().
      into_iter()
      .map(|v| 
        v.into_iter()
  ...
  fn synthesize<CS: ConstraintSystem<E::Scalar>>(
    ...
    let image_input_vars = self.image.clone()
      .into_iter()
      .enumerate()
      .map(|(i, row)| 
        row.into_iter()
          .enumerate()
          .map(|(j, val)| AllocatedNum::alloc(cs.namespace(|| format!("Input image pixel {i},{j}")), || Ok(E::Scalar::from_u128(val as u128))))
    ...
    for (i, row) in self.edited_image.clone().into_iter().enumerate() {
      ...
      for (j, pixel) in row.into_iter().enumerate() {
        let n = AllocatedNum::alloc_input(cs.namespace(|| format!("edited image entry {i} {j}")), || {
    ...
    for (i, val) in self.r.clone().into_iter().enumerate() {
      let n = AllocatedNum::alloc_input(cs.namespace(|| format!("r entry {i}")), || {
        Ok(val)
      })?;
      allocated_r.push(n);
    }
    ...
    for (i, val) in self.s.clone().into_iter().enumerate() {
      let n = AllocatedNum::alloc_input(cs.namespace(|| format!("s entry {i}")), || {
        Ok(val)
      })?;
      allocated_s.push(n);
    }
    ...
    for (i, val) in self.rTA.clone().into_iter().enumerate() {
      let n = AllocatedNum::alloc_input(cs.namespace(|| format!("rTA entry {i}")), || {
        Ok(val)
      })?;
    ...
    for (i, val) in self.As.clone().into_iter().enumerate() {
      let n = AllocatedNum::alloc_input(cs.namespace(|| format!("As entry {i}")), || {
        Ok(val)
      })?;
      allocated_As.push(n);
    }
    ...
    for (i, row) in image_input_vars.iter().enumerate() {
      ...
      for chunk in row_products.iter().zip(row_product_values.iter()).chunks(25).into_iter() {
          ...
          let inner_lc_var = AllocatedNum::alloc(cs.namespace(|| format!("Row {i} IAs inner product {level_one_chunk_idx}")), || Ok(inner_lc_val))?;
      ...
      let inner_product_var = AllocatedNum::alloc(cs.namespace(|| format!("Row {i} IAs inner product")), || Ok(level_two_lc_val))?;
      cs.enforce(
    ...
    for ((i, x), y) in IAs.iter().enumerate().zip(allocated_rTA.iter()) {
      let product = x.mul(cs.namespace(|| format!("rTIAs multiplication {i}")), y)?;
      lhs_sum_lc = lhs_sum_lc.clone() + (<E as Engine>::Scalar::ONE, product.get_variable());
      lhs_sum += self.rTA[i] * IAs_felts[i];
    }
    let rTAIAs = AllocatedNum::alloc(cs.namespace(|| format!("Freivalds LHS Result")), || Ok(lhs_sum))?;
    cs.enforce(
    ...
    for (i, row) in allocated_edited_image.iter().enumerate() {
      ...
      for chunk in row_products.iter().zip(row_product_values.iter()).chunks(25).into_iter() {
          ...
          let inner_lc_var = AllocatedNum::alloc(cs.namespace(|| format!("Row {i} Fs inner product {level_one_chunk_idx}")), || Ok(inner_lc_val))?;
      ...
      let inner_product_var = AllocatedNum::alloc(cs.namespace(|| format!("Row {i} Fs inner product")), || Ok(level_two_lc_val))?;
      cs.enforce(
    ...
    for ((i, x), y) in Fs.iter().enumerate().zip(allocated_r.iter()) {
      let product = x.mul(cs.namespace(|| format!("rTFAs multiplication {i}")), y)?;
      rhs_sum_lc = rhs_sum_lc.clone() + (<E as Engine>::Scalar::ONE, product.get_variable());
      rhs_sum += Fs_felts[i] * self.r[i];
    }
    let rTFs = AllocatedNum::alloc(cs.namespace(|| format!("Freivalds RHS Result")), || Ok(rhs_sum))?;
    cs.enforce(
